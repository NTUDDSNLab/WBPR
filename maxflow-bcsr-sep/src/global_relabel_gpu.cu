                ...
                int *Excess_total, bool *mark, bool *scanned)
{
    for (int u = 0; u < V; u++) {
        ...
        for (int i = cpu_offsets[u]; i < cpu_offsets[u + 1]; i++) {
            ...
            if (cpu_height[u] > cpu_height[v] + 1) {
                ...
                int flow;
                if (cpu_excess_flow[u] < cpu_fflows[2*i]) {
                ...
                cpu_fflows[2*i] -= flow;
                cpu_fflows[cpu_fflows[2*i + 1]] += flow; // BCSR: cpu_fflows[2*i + 1] is the reverse edge of i
            }
    ...
    CHECK(cudaMemcpy(gpu_fflows, cpu_fflows, 2* E * sizeof(int), cudaMemcpyHostToDevice));
    ...
    CHECK(cudaMemcpy(cpu_fflows, gpu_fflows, 2 * E * sizeof(int), cudaMemcpyDeviceToHost));
    ...
    while(true) {
        ...
        for (int i = idx; i < *gpu_queue_size; i += blockDim.x * gridDim.x) {
            ...
            for (int j = gpu_offsets[u]; j < gpu_offsets[u + 1]; j++) {
                ...
                if (gpu_status[v] >= 0 && gpu_fflows[2*j] > 0) {
