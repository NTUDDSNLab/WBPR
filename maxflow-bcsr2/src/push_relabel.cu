                ...
                int* gpu_avq, int* gpu_cycle)
{
    ...
    while((cpu_excess_flow[source] + cpu_excess_flow[sink]) < *Excess_total)
    {
        ...
        CHECK(cudaMemcpy(gpu_fflows, cpu_fflows, 2*E*sizeof(int), cudaMemcpyHostToDevice)); // BCSR
        ...
        CHECK(cudaMemcpy(cpu_fflows,gpu_fflows, 2*E*sizeof(int),cudaMemcpyDeviceToHost)); // BCSR
