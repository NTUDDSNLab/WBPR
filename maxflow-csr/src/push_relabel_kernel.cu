    ...
    while (cycle > 0) {
        ...
        for (unsigned int u = idx; u < V; u += blockDim.x * gridDim.x) {
            ...
            if (gpu_excess_flow[u] > 0 && gpu_height[u] < V && u != source && u != sink) {
                ...
                // Find (u, v) in backward CSR
                for (int w = 0; w < V; w++) {
                    for (int i = gpu_offsets[w]; i < gpu_offsets[w + 1]; i++) {
                ...
                if (v_dash == -1) {
                    ...
                    if (gpu_height[u] > h_dash) {
                        ...
                        /* Find flow[(u,v_dash)] in forward CSR */
                        if (!vinReverse) {
                    ...
                    int* avq)
{
    ...
    unsigned int num_iters = (int)ceilf((float)degree / (float)tileSize);
    unsigned int rdegree, num_r_iters;
    ...
    */
    /* Thead within a tile should compare the neighbors it scanned first and then
        use the parallel reduction to find the minimum height of the neighbors across the tile,
        rather than do the reduction in every iteration.
    */
    ...
    for (int i = 0; i < num_iters; i++) {
        ...
        if (i * tileSize + idx < degree) {
            ...
            // The thread finds the smaller height neighbor
            if ((gpu_fflows[v_pos] > 0) && (v != source) 
                && (gpu_height[v] < sheight[threadIdx.x])) {
    ...
    // Scan the neighbors in the backward CSR
    for (int v = 0; v < V; v++) {
        rdegree = gpu_offsets[v+1] - gpu_offsets[v];
        num_r_iters = (int)ceilf((float)rdegree / (float)tileSize);
        for (int i = idx; i < num_r_iters; i += tileSize) {
            int v_pos = gpu_offsets[v] + i * tileSize + idx;
            // Find the backward edge of u
            if ((gpu_destinations[v_pos] == u) && 
                (gpu_bflows[v_pos] > 0) && 
                (v != source) && 
                (gpu_height[v] < sheight[threadIdx.x])) 
            {
    ...
    /*  After scanning all the neighbors (forward & backward), 
        do the parallel reduction to find min */
                                    ...
                                    int *avq, int* gpu_cycle)
{
    ...
    while (cycle > 0) {
        ...
        for (int i = tileIdx; i < avq_size; i += numTilesPerGrid) {
            ...
            minV = tiled_search_neighbor<tileSize>(tile, i, sheight, svid, svidx, &vinReverse, &v_index, V, source, sink, gpu_height, gpu_excess_flow, gpu_offsets, gpu_destinations, gpu_capacities, gpu_fflows, gpu_bflows, avq); 
